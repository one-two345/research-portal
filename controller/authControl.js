...
const register = async (req, res) => {
  ...
  if (req.params.page === "register") {
    ...
    try {
      ...
      // Split team members string into an array
      const teamMembers1 = teamMembers.split(',').map(member => member.trim());
  
      // Check if project with the same title already exists
      const existingProject = await ProjectModel.findOne({ projectTitle: projectTitle });
      if (existingProject) {
        return res.json('Project with the same title already exists');
      }
